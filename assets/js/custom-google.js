    ...
    (function ($) {
      $(document).ready(function () {
        if ($('#gmap').length) {
          ...
          address_input = document.getElementById('listing_title');
          ...
          function fillInAddress() {
            ...
            // Set Zip
            var postalCode = null;
            var addProvicne = null;
            var addPhone = place.formatted_phone_number;
            var addWhatsapp = place.international_phone_number ? place.international_phone_number.replace(/\D/g, '') : '';
            var addUrl = null;
            ...
            for (var i = 0; i < place.address_components.length; i++) {
                ...
                if (component.types.indexOf('administrative_area_level_1') !== -1) {
                    addProvicne = component.long_name;
                }
            }

    
            // set the value of input field to save them to the database
            $listing_title.val(place.name);
            $listing_address.val(place.formatted_address);
            $province.val(addProvicne);
            $zipcode.val(postalCode || '');
            $phone.val(addPhone);
            $whatsapp.val(addWhatsapp || '000');
